...
// Transfer values from props to reactive variables
    ...
    fetch('assets/ne_110m_admin_0_countries.geojson').then(res => res.json()).then(countries => {
        if (globeContainer.value) {
            ...
            globe = new Globe(globeContainer.value)
                // Set height and width according to the container
                .width(width)
                .height(height)
...
function emitArc(connection: ModelsConnection) {
    if (!globe || !serverMarkerData) return;

    const start: { lat: number, lng: number } = { lat: connection.latitude, lng: connection.longitude };
    const end: { lat: number, lng: number } = { lat: serverMarkerData.lat, lng: serverMarkerData.lng };

    // Get country color based on coordinates
    const key = `${connection.latitude.toFixed(1)},${connection.longitude.toFixed(1)}`;
    if (!countryCoordinatesToColor.has(key) && connection.countryName) {
        // Use country name to generate color - same method as for polygons
        getCountryColorFromCoordinates(connection.latitude, connection.longitude, connection.countryName);
    }
