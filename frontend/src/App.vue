...
useWebSocket('ws://localhost:8080/ws',
  {
    ...
    */
    ...
    onMessage: (_, event: MessageEvent) => {
      ...
      switch (msg.type) {
        case 'attacker':
          // Emit arc from attacker to server (ipVersion 4 means address hasn't been looked up using IP API)
...
function emitArc(start: { lat: number, lng: number }, end: { lat: number, lng: number }) {
  ...
  setTimeout(() => {
    const targetRing = { lat: end.lat, lng: end.lng };
