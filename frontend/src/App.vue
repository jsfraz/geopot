...
function emitArc(start: { lat: number, lng: number }, end: { lat: number, lng: number }) {
  const arcColor = getCountryColorFromCoordinates(start.lat, start.lng);
...
<template>
  <div class="fixed top-0 left-0 w-screen h-screen overflow-hidden">
    <Transition name="fade">
      <div v-if="!isGlobeLoaded" class="absolute top-0 left-0 w-full h-full flex justify-center items-center z-10 bg-black/50">
